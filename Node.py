...
class Node:
    ...
    def calc_winScore(self):
        return (self.win+self.draw*0.05)/( 1 + self.win+self.draw+self.lose)
    ...
    def calc_selectingScore(self):
        ...
        return score
    def calc_Q(self):
        return (self.per)*(self.win)/( 1 + self.win+self.draw+self.lose) + (1-self.per)*self.value_Score # (self.win)/( 1 + self.win+self.draw+self.lose)  #
    def calc_u(self):
    ...
    def get_bestChild(self):
        ...
        if len(candidates) == 1:
            ...
            try:
                ...
                max = 0
                for i in candidates :
                    if max < self.child[i].policy_Score :
                        max = self.child[i].policy_Score
                        index = i
    ...
    def get_bestChild2(self):
        ...
        candidates = []
        for i in range(lenth):
            if max < self.child[i].calc_winScore():
                #점수가 최대값일때 index를 가진다
                max = self.child[i].calc_winScore()
                index = i
                candidates.clear()
                candidates.append(i)

            elif max == self.child[i].calc_winScore():
                #선택된 값이 최대값과 같다면 후보로 추가
                candidates.append(i)
    ...
    def print_childInfo(self):
        ...
        for i in range(lenth):
            ...
            print("vs : ", self.child[i].value_Score)
            print("q : ",self.child[i].calc_Q())
            print("u : ",self.child[i].calc_u())
        print(self.sum_childVisit())
