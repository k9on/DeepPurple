...
class OneHotEncode:
    ...
    def makeOneHot(self):
        ...
        for i in self.colomn:
            for j in self.row:
                self.position.append("%s" % j + i)
        onehot=np.eye(64)
    ...
    def makeOneHot4096(self):
        ...
        for i in range(64):
            for j in range(64):
                dic[startPosition[i]+endPosition[j]]=onehot4096[(i+1)*(j+1)-1]
                self.position4096.append(startPosition[i]+endPosition[j])
        return dic
    ...
    def uciMoveToOnehot2(self,move):
        print(self.onehot4096[move[:4]])
        #uci명령어 move를 입력 받아 4096bit의 one hot을 반환
        return np.ndarray.tolist(self.onehot4096[move[:4]])
